...
class InteractiveForecast:
    ...
    def create_figure(self):
                ...
                if not row.empty:
                    ...
                    fig.add_shape(
                        ...
                        line=dict(
                            ...
                            width=12,  # Reduced width to prevent overlap
                        ),
        ...
        # Update layout with expanded y-axis range
        y_min, y_max = np.min(self.y_noisy), np.max(self.y_noisy)

        # Get the range of intervals for additional margin calculation
        interval_ranges = []
        if self.condition != "control":
            confidence_level = "95" if "95" in self.condition else "80"
            lower_col = f"lower_{confidence_level}"
            ...
            for x_pos in self.forecast_x_positions:
                ...
                if not row.empty:
                    interval_ranges.append(row[upper_col].values[0] - row[lower_col].values[0])

        # Calculate margins based on both data range and interval sizes
        data_range = y_max - y_min
        interval_range = max(interval_ranges) if interval_ranges else data_range * 0.2
        margin = max(data_range * 0.2, interval_range)

        # Set y-axis range to accommodate double the interval size
        y_axis_min = y_min - margin * 2
        y_axis_max = y_max + margin * 2
