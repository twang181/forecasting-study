...
def generate_time_series():
    ...
    y_clean = 50 + 300 * (x / 50) ** k  # Prediction values (without noise)
    noise = np.random.normal(0, 3, len(x))  # Gaussian noise
    y_noisy = y_clean + noise  # Actual sales with noise
...
def generate_forecast():
    future_x = np.arange(51, 59)  # Future periods
    k = 1.5  # Same exponent as original data
    future_y = 50 + 300 * (future_x / 50) ** k  # Future values (no noise)
...
class InteractiveForecast:
    def __init__(self):
        ...
        # Generate time-series and forecast data
        self.x, self.y_clean, self.y_noisy = generate_time_series()
        self.y_lower, self.y_upper = calculate_prediction_interval(self.y_noisy)
        self.future_x, self.future_y = generate_forecast()

        # **Plot actual noisy data points (black dots)**
        self.ax.scatter(self.x, self.y_noisy, color='black', s=10, label="Observed Sales")

        # **Remove confidence intervals for first 50 points**
        # (Simply don't plot them)

        # **Y-axis scaling**: Adjusting to ensure upper bounds are visible
        min_val = min(np.min(self.y_lower), np.min(self.future_y)) - 20
        max_val = max(np.max(self.y_upper), np.max(self.future_y)) + 20
        ...
        self.forecast_lower, self.forecast_upper = calculate_prediction_interval(self.future_y)
