    ...
    def setup_callbacks(self):
       ...
       # def store_participant_info(search):
        ...
        def update_plot(n_clicks, relayout_data, click_data, search, user_intervals, current_fig, click_buffer):
            ...
            if triggered_id == 'url':
                # Extract or assign participant ID
                participant_id = str(uuid.uuid4())
                if search:
                    parsed = urllib.parse.parse_qs(search.lstrip('?'))
                    if 'id' in parsed:
                ...
                return self.create_figure(), "", user_intervals, click_buffer, self.get_nudge_text(), {
                    'id': participant_id,
                    'condition': condition
                }

            elif triggered_id == 'save-button' and n_clicks:
                self.save_data(user_intervals)

                # Switch series and check if done
                ...
                return self.create_figure(), "Data saved successfully!", {}, {
                    'points': []}, self.get_nudge_text(), dash.no_update
