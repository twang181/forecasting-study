...
class InteractiveForecast:
    ...
    def switch_to_new_series(self):
        """Switch to a new random time series and update the plot correctly."""
        self.ax.clear()  # Clear the previous plot

        if self.cursor is not None:
            self.cursor.remove()  # Remove old cursor
            self.cursor = None

        for artist in self.ax.texts:  # Remove all annotations
            artist.remove()

        # Reset user intervals if in control condition
        if self.condition == "control":
            self.user_intervals = {}

        # Pick a new random series
        self.series_index = np.random.randint(1, n_series + 1)
        ...
        # 🔄 **Ensure forecast intervals are re-initialized**
        if self.condition != "control":
            self.initialize_forecasts()

        # ✅ **Force Matplotlib to recalculate the y-limits**
        y_min = min(self.y_noisy)
        y_max = max(self.y_noisy)

        # Use forecast intervals only if they exist
        if hasattr(self, 'forecast_lower') and hasattr(self, 'forecast_upper'):
            y_min = min(y_min, min(self.forecast_lower))
            y_max = max(y_max, max(self.forecast_upper))

        margin = (y_max - y_min) * 0.2  # Add 20% margin
        self.ax.set_ylim(y_min - margin, y_max + margin)  # 🔥 **This forces new y-limits**
        ...
        # ✅ **Fix: Reinitialize cursor for new scatter points**
        self.cursor = mplcursors.cursor(scatter_plot, hover=True)
        self.cursor.connect("add", lambda sel: sel.annotation.set_text(f"({sel.target[0]:.0f}, {sel.target[1]:.2f})"))

        # ✅ **Force full plot update**
