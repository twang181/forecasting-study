...
class InteractiveForecast:
    ...
    def initialize_forecasts(self):
        ...
        # Adjust the y-axis and layout for all conditions
        self.adjust_y_axis_based_on_forecast()  # This will apply to all conditions

        # Adjust the layout manually (no tight layout)
        self.fig.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)  # Adjust margins
    ...
    class InteractiveForecast:
        def __init__(self):
            ...
            self.cursor.connect("add",
                                lambda sel: sel.annotation.set_text(f"({sel.target[0]:.0f}, {sel.target[1]:.2f})"))
        ...
        def add_bar(self, x_pos):
            ...
            bar = \
            self.ax.bar(x_pos, upper - lower, bottom=lower, width=0.5, color='skyblue', edgecolor=None, alpha=0.4)[
                0]
            self.user_intervals[x_pos][2] = bar
            self.fig.canvas.draw()
        ...
        def switch_to_new_series(self):
            ...
            if self.cursor is not None:
                self.cursor.remove()  # Remove previous cursor
                self.cursor = None  # Reset to None

            for artist in self.ax.texts:  # Clear all old annotations
                artist.remove()

            # Don't reset user intervals if in control condition
            if self.condition == "control":
                # Keep user intervals intact when in control
                pass
            else:
                self.user_intervals = {}  # Reset intervals if not in control
            ...
            # Adjust y-axis limits based on the interval method
            self.adjust_y_axis_based_on_forecast()  # Ensure y-axis is adjusted when switching series
            ...
            # Manually adjust layout (no tight layout)
            self.fig.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)  # Adjust margins

            # ✅ Fix: Reinitialize cursor ONLY for scatter points (not the whole line)
            self.cursor = mplcursors.cursor(scatter_plot, hover=True)
            self.cursor.connect("add",
                                lambda sel: sel.annotation.set_text(f"({sel.target[0]:.0f}, {sel.target[1]:.2f})"))

            # ✅ Fix: Force the canvas to redraw without past values
            self.fig.canvas.draw()

        def show_plot(self):
            plt.show()

        def adjust_y_axis_based_on_forecast(self):
            """Adjust y-axis limits based on forecast intervals."""
            if self.condition != "control" and hasattr(self, 'forecast_lower') and hasattr(self, 'forecast_upper'):
                # If forecast intervals exist, use doubling interval method
                interval_range = max(self.forecast_upper) - min(self.forecast_lower)  # Interval width
                point_forecast = np.mean([self.forecast_lower, self.forecast_upper])  # Average point forecast

                # Calculate new min and max for the y-axis
                lower_limit = point_forecast - 4 * interval_range
                upper_limit = point_forecast + 4 * interval_range

                self.ax.set_ylim(lower_limit, upper_limit)
            else:
                # Default adjustment for "control" condition
                y_range = max(self.y_noisy) - min(self.y_noisy)
                margin = y_range * 0.2  # 20% margin
                self.ax.set_ylim(min(self.y_noisy) - margin, max(self.y_noisy) + margin)
