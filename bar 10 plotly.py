...
class InteractiveForecast:
        ...
        if self.condition != "control":
            ...
            for x_pos in self.forecast_x_positions:
                ...
                if not row.empty:
                    ...
                    # Add interval as a rectangle
                    fig.add_trace(go.Scatter(
                        x=[x_pos, x_pos],
                        y=[lower, upper],
                        mode='lines',
                        line=dict(width=20, color='rgba(135, 206, 235, 0.4)'),
                        showlegend=False,
                        hoverinfo='y'
                    ))
    ...
    def setup_layout(self):
        self.app.layout = dbc.Container([
            ...
            dcc.Graph(
                ...
                config={
                    'modeBarButtonsToAdd': ['drawopenpath', 'eraseshape'] if self.condition == "control" else [],
                    'scrollZoom': False
                }
            ),
    ...
    def setup_callbacks(self):
        ...
        def update_plot(n_clicks, relayout_data, user_intervals):
            ...
            if triggered_id == 'save-button' and n_clicks:
                ...
                if 'shapes' in relayout_data:
                    # Update user intervals based on drawn shapes
                    user_intervals = self.process_shapes(relayout_data['shapes'])

            return self.create_figure(), "", user_intervals
    ...
    def process_shapes(self, shapes):
        ...
        for shape in shapes:
            if shape['type'] == 'line':
                x = shape['x0']
                y = shape['y0']
                if x in self.forecast_x_positions:
                    if x not in intervals:
                        intervals[x] = {'lower': None, 'upper': None}
                    if intervals[x]['lower'] is None:
                        intervals[x]['lower'] = y
                    else:
                        intervals[x]['upper'] = y
                        if intervals[x]['lower'] > intervals[x]['upper']:
                            intervals[x]['lower'], intervals[x]['upper'] = intervals[x]['upper'], intervals[x]['lower']
        return intervals
