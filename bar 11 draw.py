        ...
        if self.condition != "control":
            ...
            for x_pos in self.forecast_x_positions:
                ...
                if not row.empty:
                    ...
                    # Add interval as a draggable bar
                    fig.add_trace(go.Scatter(
                        x=[x_pos, x_pos],
                        y=[lower, upper],
                        mode='lines',
                        line=dict(width=20, color='rgba(135, 206, 235, 0.7)'),
                        showlegend=False,
                        hoverinfo='y',
                        name=f'Interval_{x_pos}',
                        hoverlabel=dict(bgcolor='rgba(135, 206, 235, 0.7)'),
            ...
            if triggered_id == 'save-button' and n_clicks:
                ...
                if 'shapes' in relayout_data:
                    for shape in relayout_data['shapes']:
                        if shape['type'] == 'line':
                            x_pos = shape['x0']
                            if x_pos in self.forecast_x_positions:
                                y0, y1 = shape['y0'], shape['y1']

                                # Update the interval bar
                                for trace in current_fig['data']:
                                    if trace['name'] == f'Interval_{x_pos}':
                                        trace['y'] = [min(y0, y1), max(y0, y1)]

                                        # Store the updated values
                                        if str(x_pos) not in user_intervals:
                                            user_intervals[str(x_pos)] = {}
                                        user_intervals[str(x_pos)]['lower'] = min(y0, y1)
                                        user_intervals[str(x_pos)]['upper'] = max(y0, y1)
                                        break
